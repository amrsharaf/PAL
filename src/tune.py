...
class Data(object):
    def __init__(self, train_file, test_file, dev_file, emb_file, budget, max_seq_len, max_vocab_size, emb_size,
                 model_name):
    ...
    def __call__(self):
        ...
        game = initialize_game(train_file=self.train_file, test_file=self.test_file, dev_file=self.dev_file,
                               emb_file=self.emb_file, budget=self.budget, max_seq_len=self.max_seq_len,
                               max_vocab_size=self.max_vocab_size, emb_size=self.emb_size, model_name=self.model_name)
...
def main():
    ...
    data = Data(train_file=lang.train, test_file=lang.test, dev_file=lang.dev, emb_file=lang.emb, budget=args.budget,
                max_seq_len=args.max_seq_len, max_vocab_size=args.max_vocab_size, emb_size=args.emb_size,
                model_name=args.model_name)
    create_model = CreateModel(max_len=args.max_len, input_dim=args.input_dim, output_dim=args.output_dim,
                               embedding_matrix=args.embedding_matrix)
    best_run, best_model = optim.optimize(model=create_model, data=data, algo=tpe.suggest, max_evals=5, tirals=Trials())
